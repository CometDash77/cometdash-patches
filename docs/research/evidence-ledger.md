...
### Official Voice over translation Phase 2 model

- The [Phase 2 source report](./official-voice-over-translation.md) fixes the research corpus at `morphe-patches@e12088c89942f5d637a824ce81643a28b86fb851`; collection-time `main` matched that revision and the recursive tree returned `2654` entries with `truncated=false`.
- [`VoiceOverTranslationPatch.kt`](https://github.com/MorpheApp/morphe-patches/blob/e12088c89942f5d637a824ce81643a28b86fb851/patches/src/main/kotlin/app/morphe/patches/youtube/video/voiceovertranslation/VoiceOverTranslationPatch.kt#L39-L108) packages the feature resources/settings and routes shared video-ID, playback-time, overlay-button and legacy-control hooks to the extension. [`VotOriginalVolumeBytecodePatch.kt`](https://github.com/MorpheApp/morphe-patches/blob/e12088c89942f5d637a824ce81643a28b86fb851/patches/src/main/kotlin/app/morphe/patches/youtube/video/voiceovertranslation/VotOriginalVolumeBytecodePatch.kt#L25-L39) injects the AudioSink multiplier and AudioTrack capture calls.
- [`TranscriptFetcher.java`](https://github.com/MorpheApp/morphe-patches/blob/e12088c89942f5d637a824ce81643a28b86fb851/extensions/youtube/src/main/java/app/morphe/extension/youtube/patches/voiceovertranslation/TranscriptFetcher.java#L48-L215) obtains/normalizes JSON3 captions through Innertube with direct timedtext fallback. [`TranscriptTranslator.java`](https://github.com/MorpheApp/morphe-patches/blob/e12088c89942f5d637a824ce81643a28b86fb851/extensions/youtube/src/main/java/app/morphe/extension/youtube/patches/voiceovertranslation/TranscriptTranslator.java#L190-L321) batches around the playhead and publishes partial/completed same-shape snapshots for Google, MyMemory or OpenRouter.
- The translator uses static/volatile shared state and an atomic session counter; generation protects completed-batch callbacks but not every streamed partial callback. [`VoiceOverTranslationPatch.java`](https://github.com/MorpheApp/morphe-patches/blob/e12088c89942f5d637a824ce81643a28b86fb851/extensions/youtube/src/main/java/app/morphe/extension/youtube/patches/voiceovertranslation/VoiceOverTranslationPatch.java#L456-L519) checks video/language before list replacement but does not freeze a full per-video provider/config generation.
- [`TtsEngine.java`](https://github.com/MorpheApp/morphe-patches/blob/e12088c89942f5d637a824ce81643a28b86fb851/extensions/youtube/src/main/java/app/morphe/extension/youtube/patches/voiceovertranslation/TtsEngine.java#L118-L344), [`TtsCache.java`](https://github.com/MorpheApp/morphe-patches/blob/e12088c89942f5d637a824ce81643a28b86fb851/extensions/youtube/src/main/java/app/morphe/extension/youtube/patches/voiceovertranslation/TtsCache.java#L25-L94) and [`TtsPrefetcher.java`](https://github.com/MorpheApp/morphe-patches/blob/e12088c89942f5d637a824ce81643a28b86fb851/extensions/youtube/src/main/java/app/morphe/extension/youtube/patches/voiceovertranslation/TtsPrefetcher.java#L126-L296) provide System/Edge playback, in-memory production speech caching, preview-only disk caching and distance/backoff-based Edge prefetch.
- [`VotOriginalVolumePatch.java`](https://github.com/MorpheApp/morphe-patches/blob/e12088c89942f5d637a824ce81643a28b86fb851/extensions/youtube/src/main/java/app/morphe/extension/youtube/patches/voiceovertranslation/VotOriginalVolumePatch.java#L22-L100) immediately applies and reenforces a clamped multiplier every 50 ms; the inspected source does not implement a fade or one lifecycle-owned audible-speech ducking token.
- Evidence acquisition preserved failures: configured `gh api` returned 401, anonymous API later hit its rate limit, some raw calls reset/timed out and Web search returned 404. Fixed raw blobs and the same-SHA codeload archive succeeded; failed calls were not interpreted as missing source or runtime behavior.
- Candidate self-check `pwsh -NoProfile -File tools/sync_upstream_docs.ps1 -Check` remained blocked at `sync_upstream_docs.ps1:120`: the configured GitHub credential produced HTTP 401 `Bad credentials`. This check is recorded as failed, not PASS; Phase 2 does not rely on the vendored upstream-documentation snapshot for official VoT source claims.
...
- Exact official VoT fingerprint matches, target owners and callback/runtime behavior on YouTube `21.04.223`; the fixed source defines match rules but Phase 2 did not inspect the APK.
- Actual Android System TTS behavior, Edge voice/service/protocol success, and current Google/MyMemory/OpenRouter server responses.
- Whether the source-visible static translator state and partially guarded streamed callbacks can publish stale provider/service results under concurrent lifecycle changes.
- Final Logger redaction, visibility and persistence behavior for provider error bodies and translated-text debug fragments.
