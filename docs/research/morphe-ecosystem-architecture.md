...
| 2 | [`Apktool`](https://github.com/MorpheApp/Apktool/tree/a3f22c3ab59860ee30312da8869e818648c24d38) | Yes | `main` | `a3f22c3ab59860ee30312da8869e818648c24d38` | Excluded | Direct 组件的固定依赖没有 `Apktool`；Patcher 使用 ARSCLib 的 `ApkModule*`，而该 fork 的 [`build.gradle.kts`](https://github.com/MorpheApp/Apktool/blob/a3f22c3ab59860ee30312da8869e818648c24d38/build.gradle.kts) 是独立 Apktool 工程配置。 |
| 3 | [`ARSCLib`](https://github.com/MorpheApp/ARSCLib/tree/d003b5ff1ca91fb8c5105619cf1108b450387061) | Yes | `main` | `d003b5ff1ca91fb8c5105619cf1108b450387061` | Supporting | Patcher 的 [`build.gradle.kts`](https://github.com/MorpheApp/morphe-patcher/blob/b69536fd33b69a1d1b2643068941f1052cf51708/build.gradle.kts) 声明 ARSCLib，[`ArsclibResourceCoder`](https://github.com/MorpheApp/morphe-patcher/blob/b69536fd33b69a1d1b2643068941f1052cf51708/src/main/kotlin/app/morphe/patcher/resource/coder/ArsclibResourceCoder.kt) 调用其 `ApkModule` decode/encode API；它提供资源读写能力，但不拥有 Source/Patch/APK 生命周期编排边界。 |
...
| 18 | [`morphe-website`](https://github.com/MorpheApp/morphe-website/tree/57a6d8cb9541101d09fc73fef8c1e0c3304e7dc7) | No | `main` | `57a6d8cb9541101d09fc73fef8c1e0c3304e7dc7` | Excluded | official patches release workflow 仅在发布后触发网站 deploy；网站的 [`package.json`](https://github.com/MorpheApp/morphe-website/blob/57a6d8cb9541101d09fc73fef8c1e0c3304e7dc7/package.json) 和 [`wrangler.toml`](https://github.com/MorpheApp/morphe-website/blob/57a6d8cb9541101d09fc73fef8c1e0c3304e7dc7/wrangler.toml) 不生成/承载本报告定义的 Source metadata 或 `.mpp`。 |
...
- **Supporting**：Direct 组件在冻结源码/配置中明确导入或调用它，但它只提供共享 API、resource/DEX model 或 transport，不决定生命周期阶段的输入/输出。
- **Excluded**：完整依赖与调用追踪没有把它接入本报告定义的 Patch 生命周期；仓库名、组织归属、fork 身份或发布后旁路动作不能代替源码边。

特别容易误判的项目：`Apktool` 没有被本次冻结 Direct 构建引用；资源路径实际由 Patcher 的 [`ArsclibResourceCoder`](https://github.com/MorpheApp/morphe-patcher/blob/b69536fd33b69a1d1b2643068941f1052cf51708/src/main/kotlin/app/morphe/patcher/resource/coder/ArsclibResourceCoder.kt) 调用 ARSCLib。`multidexlib2` 自己依赖 smali 不等于 Direct 组件依赖它。`morphe-website` 收到 release 后的 deploy dispatch，但不生成 `patches-bundle.json` 或发布 `.mpp`；这些职责由 release config、`changelog` plugin 和 GitHub Release 承担。
...
| Supporting ([`morphe-patches-library`](https://github.com/MorpheApp/morphe-patches-library/blob/9e555a2273533ef13e51db70a55d3fd544752756/patch-library/src/main/kotlin/app/morphe/util/PatchListGenerator.kt), [ARSCLib integration](https://github.com/MorpheApp/morphe-patcher/blob/b69536fd33b69a1d1b2643068941f1052cf51708/src/main/kotlin/app/morphe/patcher/resource/coder/ArsclibResourceCoder.kt), [smali integration](https://github.com/MorpheApp/morphe-patcher/blob/b69536fd33b69a1d1b2643068941f1052cf51708/src/main/kotlin/app/morphe/patcher/patch/BytecodePatchContext.kt), [jadb integration](https://github.com/MorpheApp/morphe-library/blob/a5b1fb512306d497cad8a13c0399a5fb28553522/src/commonMain/kotlin/app/morphe/library/installation/command/AdbShellCommandRunner.kt)) | shared helpers/metadata generator | - | model/transport support | resource/DEX support | - | ADB transport support |
...
flowchart LR
    ...
    E --> F["PatchListGenerator -> patches-list.json"]
    F --> G["semantic-release + changelog"]
    G --> H["patches-bundle.json + GitHub Release .mpp"]

    H --> I["Android Manager Source normalization / metadata fetch"]
    H --> J["Desktop RemotePatchSource"]
    I --> K["download .mpp -> patches.jar"]
    J --> L["download .mpp"]
    K --> M["loadPatchesFromDex + Android selection"]
    L --> N["loadPatchesFromJar + Desktop selection"]
    M --> O["morphe-patcher PatcherContext"]
    N --> O
    P["Original APK"] --> O
    O --> Q["Patched DEX/resources"]
    Q --> R["applyTo copy of original APK"]
    R --> S["Signed/output APK"]
    S --> T["Manager installers"]
    S --> U["morphe-library AdbInstaller via Desktop CLI"]
    T --> V["Install result"]
    U --> V
```
...
| Artifact | Producer | Consumer | 语义与证据 |
| --- | --- | --- | --- |
| Source URL | 用户/配置 | Manager `normalizeRemoteBundleUrl`; Desktop `RemotePatchSourceFactory.parse` | Source 是 metadata/asset 的定位入口，不是 APK。Manager 将 GitHub repo URL 变为 raw `patches-bundle.json`；Desktop 将输入解析为 provider + `owner/repo`。证据：[`PatchBundleRepository.kt`](https://github.com/MorpheApp/morphe-manager/blob/a2c3d31bd7ab42e6bc4b9dd528ed856fc72fb948/app/src/main/java/app/morphe/manager/domain/repository/PatchBundleRepository.kt)、[`RemotePatchSourceFactory.kt`](https://github.com/MorpheApp/morphe-desktop/blob/2f5ce39adc26d4b3e7debe44445eddcaf887bffa/src/main/kotlin/app/morphe/engine/patches/RemotePatchSourceFactory.kt)。 |
| `patches-bundle.json` | `changelog.prepare` + semantic-release config | Manager `JsonPatchBundle.getLatestInfo`; Desktop `fetchLatestFromManifest` | 包含 version、description、`.mpp` download URL 等 release metadata；不是 Patch executable。证据：[`prepare.js`](https://github.com/MorpheApp/changelog/blob/caa1e931730f097bb6c4dee636b01c2c24ccd72d/lib/prepare.js)、template [`.releaserc`](https://github.com/MorpheApp/morphe-patches-template/blob/93ade63a00a4b5954c63af78dbd9d8e6ec4f95fe/.releaserc)。 |
| `patches-list.json` | `PatchListGenerator` | README/third-party tooling；本次未发现 Manager/Desktop runtime consumer | Patch 名称、默认值、dependency、compatibility、option 的发布清单。证据：shared [`PatchListGenerator.kt`](https://github.com/MorpheApp/morphe-patches-library/blob/9e555a2273533ef13e51db70a55d3fd544752756/patch-library/src/main/kotlin/app/morphe/util/PatchListGenerator.kt)。 |
| `.mpp` | `PatchesPlugin` JAR task + `buildAndroid` | Manager/Desktop Patch loader | ZIP/JAR container，包含 JVM class、manifest、extension `.mpe` resources 和 Android DEX；不是 APK。证据：[`PatchesPlugin.kt`](https://github.com/MorpheApp/morphe-patches-gradle-plugin/blob/52be641ed3b965a20c33bd43e0cbe9efd308bc64/src/main/kotlin/app/morphe/patches/gradle/PatchesPlugin.kt)。 |
| Loaded `Patch` | Patcher `PatchLoader.Jar`/`.Dex` | Manager/Desktop selection、`Patcher.plusAssign` | 从 public static field/zero-arg public static method 反射得到、有名称的 `Patch` object；携带 dependency、compatibility、options 和 execute/finalize block。证据：[`Patch.kt`](https://github.com/MorpheApp/morphe-patcher/blob/b69536fd33b69a1d1b2643068941f1052cf51708/src/main/kotlin/app/morphe/patcher/patch/Patch.kt)。 |
| Original APK | 用户选择的 local/installed app | Manager `PatcherWorker` / Desktop `PatchEngine` / Patcher | 只作为 target input。host 复制它，再把 mutation result 应用到副本。证据：Manager [`Session.kt`](https://github.com/MorpheApp/morphe-manager/blob/a2c3d31bd7ab42e6bc4b9dd528ed856fc72fb948/app/src/main/java/app/morphe/manager/patcher/Session.kt)、Desktop [`PatchEngine.kt`](https://github.com/MorpheApp/morphe-desktop/blob/2f5ce39adc26d4b3e7debe44445eddcaf887bffa/src/main/kotlin/app/morphe/engine/PatchEngine.kt)。 |
| Transformed APK | `PatcherResult.applyTo` on copied original APK | host signer | 替换/增加资源、删除 staged entries、写 patched DEX 并 realign。证据：[`ApkUtils.kt`](https://github.com/MorpheApp/morphe-patcher/blob/b69536fd33b69a1d1b2643068941f1052cf51708/src/main/kotlin/app/morphe/patcher/apk/ApkUtils.kt)。 |
| Signed/output APK | Manager `KeystoreManager.sign` / Desktop `PatchEngine.patch` | export/install | Patcher `ApkUtils.signApk` 产生最终 signed file，host 决定输出路径。证据：Manager [`KeystoreManager.kt`](https://github.com/MorpheApp/morphe-manager/blob/a2c3d31bd7ab42e6bc4b9dd528ed856fc72fb948/app/src/main/java/app/morphe/manager/domain/manager/KeystoreManager.kt)、Desktop [`PatchEngine.kt`](https://github.com/MorpheApp/morphe-desktop/blob/2f5ce39adc26d4b3e7debe44445eddcaf887bffa/src/main/kotlin/app/morphe/engine/PatchEngine.kt)。 |
| Install result | Manager installer implementation / `morphe-library` installer | UI/CLI | Android 路径返回 system/Shizuku/root/external outcome；Desktop CLI 的 `AdbInstaller` 返回 `Success` 或 `Failure`. 证据：Manager [`SessionInstaller.kt`](https://github.com/MorpheApp/morphe-manager/blob/a2c3d31bd7ab42e6bc4b9dd528ed856fc72fb948/app/src/main/java/app/morphe/manager/domain/installer/SessionInstaller.kt)、library [`AdbInstaller.kt`](https://github.com/MorpheApp/morphe-library/blob/a5b1fb512306d497cad8a13c0399a5fb28553522/src/commonMain/kotlin/app/morphe/library/installation/installer/AdbInstaller.kt)。 |
